(function(){
  ...
  function renderScenario(sc,panel){
    ...
    panel.append(make('h3','risk-scenario-title',`S${sc.id} — ${sc.name}`));
    panel.append(make('p','risk-scenario-context',[
      'Full traffic load','46°C',
      sc.batteryState==='charging'?'Batteries charging':'Batteries charged'].join(' · ')));
    const metrics=make('div','risk-metrics');
    for(const [label,key] of [['Power','power'],['Cooling','cooling'],['Rectifier','rectifier']]){
      const cell=make('div','risk-metric '+(sc.flags[key]?'is-risk':'is-safe'));
      cell.append(make('span','',label),make('strong',sc.flags[key]?'negative':'positive',sc.flags[key]?'RISK':'SAFE'));
      metrics.append(cell);
    }
    ...
    for(const [label,value,unit,key] of [
      ...
      const row=make('div','risk-margin');
      row.append(make('span','',label),make('strong',sc.flags[key]?'negative':'',`${fmt(value,2)} ${unit}`));
      margins.append(row);
    }
    panel.append(margins);
    const inputDetails=make('details','risk-inputs');
    inputDetails.append(make('summary','','Engineering parameters'));
    ...
    inputDetails.append(inputs);panel.append(inputDetails);
  }
  function renderSite(raw){
    ...
    const snapshot=make('div','risk-asset-grid');
    snapshot.append(field('Power source',power),
      field('AC1',`${fmt(raw.ac1CapacityBtu/1000,0)}k Btu/h`),
      field('AC2',raw.ac2CapacityBtu?`${fmt(raw.ac2CapacityBtu/1000,0)}k Btu/h`:'—'),
      field('Rectifier',`${raw.rectifierCapacityKw} kW`),
      field('Battery',`${raw.batteriesCapacityAh*raw.numStrings} Ah lead-acid`),
      field('Site load',`${fmt(raw.telecomLoadTotalKw||raw.telecomLoadAllKw)} kW`));
    const more=make('details','risk-site-inputs');
    more.append(make('summary','','More site inputs'));
    const extra=make('div','risk-detail-grid');
    extra.append(field('Location',raw.location||'—'),field('Type',raw.shelterType==='Outdoor'?'Outdoor cabinet':'Shelter'),
    ...
    content.replaceChildren(snapshot,more);
    const scenarioHead=make('div','risk-scenario-head');
    scenarioHead.append(make('h3','risk-section-title','Engineering scenarios'));
    ...
    scenarioHead.append(tabs);
    content.append(scenarioHead,panel);renderScenario(result.scenarios[0],panel);
  }
